package org.simplicite.chess.eval;

import org.simplicite.chess.move.BBMagicAttacks;
 ...
 * structure, king safety, piece placement, and rook-pawn cooperation (viz., rooks on open files, rooks supporting passed pawns).
 */
public class CompleteEvaluator implements Evaluator
{
	...
	public int eval(Board b) {
		...
		int numWhitePieces = Long.bitCount(b.white_knights) + Long.bitCount(b.white_bishops) + Long.bitCount(b.white_rooks) + Long.bitCount(b.white_queens);
		int numBlackPieces = Long.bitCount(b.black_knights) + Long.bitCount(b.black_bishops) + Long.bitCount(b.black_rooks) + Long.bitCount(b.black_queens);
		...
		if (b.whitePieceMaterial() + 100*numWhitePawns > b.blackPieceMaterial() + 100*numBlackPawns)
		...
		while (temp != 0L)
		{
			...
			else if ((BBMagicAttacks.whitePawn[loc + 8] & b.black_pawns)!=0 && (BACKWARD_WHITE[loc] & b.white_pawns)==0)
		...
		while (temp != 0L)
		{
			...
			if ((ISOLATED_BLACK[loc] & b.black_pawns) == 0)
				score += PENALTY_ISOLATED_PAWN;
			else if ((BBMagicAttacks.blackPawn[loc - 8] & b.white_pawns)!=0 && (BACKWARD_BLACK[loc] & b.black_pawns)==0)
				score += PENALTY_BACKWARD_PAWN;
		...
		while (temp != 0L)
		{
			...
			{
				long pawnOnSameFile = Long.highestOneBit(BBUtils.maskFile[BBUtils.getLocCol(loc)] & whitePassedPawns);
				if (BBUtils.getLocFromBitboard(rook) < BBUtils.getLocFromBitboard(pawnOnSameFile))
		...
		while (temp != 0L)
		{
			...
			{
				long pawnOnSameFile = Long.highestOneBit(BBUtils.maskFile[BBUtils.getLocCol(loc)] & blackPassedPawns);
				if (BBUtils.getLocFromBitboard(rook) < BBUtils.getLocFromBitboard(pawnOnSameFile))
					score -= BONUS_ROOK_DEFENDING_PASSED_PAWN;
			}
		...
		else
		{
			...
			score += BONUS_STRONG_SHIELD * Long.bitCount(STRONG_SAFE_WHITE[whiteKingLoc] & b.white_pawns);
			score += BONUS_WEAK_SHIELD * Long.bitCount(WEAK_SAFE_WHITE[whiteKingLoc] & b.white_pawns);

			// black king
			score -= KING_POS_M_BLACK[blackKingLoc];
			score -= BONUS_STRONG_SHIELD * Long.bitCount(STRONG_SAFE_BLACK[blackKingLoc] & b.black_pawns);
			score -= BONUS_WEAK_SHIELD * Long.bitCount(WEAK_SAFE_BLACK[blackKingLoc] & b.black_pawns);
		}
