...
| Library (LIB script) | `simfunpack-chess` | `lib/simfunpack-chess.jar`, the Java chess engine (built from `others/`) |
...
This produces `others/target/simfunpack-chess.jar` (engine classes + the
`opening/book.txt` opening book). Deploy it as the `simfunpack-chess` library
(`lib/simfunpack-chess.jar`). `SimGameChess` and `SimChess` call the engine and
the SSE API by reflection, so the module's own build has no compile dependency
on either.
